        ...
        <form action="{{ route('admin.portfolio.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div>
                ...
                <select name="category" required class="w-full px-3 py-2 rounded-lg border border-slate-300 text-xs focus:border-amber-500 outline-none bg-white">
                    ...
                    <option value="wardrobe">Sliding Wardrobe</option>
    ...
    <div class="bg-white p-6 sm:p-7 rounded-2xl border border-slate-200 shadow-xs space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            ...
            <div class="flex items-center gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 sm:overflow-visible sm:pb-0 text-xs shrink-0">
                ...
                <a href="{{ route('admin.portfolio.index', ['category' => 'kitchen']) }}" 
                   class="px-3 py-1.5 rounded-lg whitespace-nowrap {{ request('category') === 'kitchen' ? 'bg-slate-900 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                    Kitchens
                </a>
                <a href="{{ route('admin.portfolio.index', ['category' => 'wardrobe']) }}" 
                   class="px-3 py-1.5 rounded-lg whitespace-nowrap {{ request('category') === 'wardrobe' ? 'bg-slate-900 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                    Wardrobes
                </a>
                <a href="{{ route('admin.portfolio.index', ['category' => 'glazing']) }}" 
                   class="px-3 py-1.5 rounded-lg whitespace-nowrap {{ request('category') === 'glazing' ? 'bg-slate-900 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                    Aluminium
                </a>
                <a href="{{ route('admin.portfolio.index', ['category' => 'ceiling']) }}" 
                   class="px-3 py-1.5 rounded-lg whitespace-nowrap {{ request('category') === 'ceiling' ? 'bg-slate-900 text-white font-bold' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                    Ceilings
                </a>
            </div>
        </div>
